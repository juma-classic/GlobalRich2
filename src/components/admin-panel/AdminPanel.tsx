...
export const AdminPanel: React.FC<AdminPanelProps> = ({ isOpen, onClose }) => {
    ...
    const loadAdminAccounts = () => {
        try {
            const stored = localStorage.getItem('globaltrades_admin_accounts');
            if (stored) {
                ...
                localStorage.setItem('globaltrades_admin_accounts', JSON.stringify(defaultAdmins));
            }
        } catch (error) {
    ...
    const loadPremiumAccounts = async () => {
        try {
            ...
            if (response.ok) {
                ...
                const combined = [
                    ...(data.autoRich2026 || []).map((acc: string) => ({ account: acc, bot: 'AutoRich 2026' })),
                    ...(data.divergentPro || []).map((acc: string) => ({ account: acc, bot: 'Divergent Pro' })),
                ];
    ...
    const saveAdminAccounts = (accounts: string[]) => {
        try {
            localStorage.setItem('globaltrades_admin_accounts', JSON.stringify(accounts));
    ...
    const filteredAccounts = adminAccounts.filter(acc => acc.toLowerCase().includes(searchTerm.toLowerCase()));
                                    ...
                                    filteredAccounts.map(account => (
                                        <div key={account} className='account-item'>
                                            <div className='account-info'>
                                                <span
                                                    className={`account-badge ${account.startsWith('CR') ? 'real' : 'demo'}`}
                                                >
                            ...
                            >
                                ...
                                <ol style={{ margin: '0', paddingLeft: '1.5rem', color: '#6b7280', fontSize: '13px' }}>
                                    ...
                                    <li>
                                        Open{' '}
                                        <code
                                            style={{ background: '#e5e7eb', padding: '2px 6px', borderRadius: '4px' }}
                                        >
                                            public/premium-whitelist.json
                                        </code>
                                    </li>
                                    <li>
                                ...
                                {isLoading ? (
                                    ...
                                    <div className='empty-state'>
                                        {premiumSearchTerm
                                            ...
                                            : 'No premium accounts whitelisted yet'}
                                    </div>
                                ) : (
